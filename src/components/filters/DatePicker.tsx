...
function QuarterGrid({ st, patch }: { st: DateState; patch: Patch }) {
  ...
  return (
    <>
      ...
      <div className="qgrid">
        {qs.map(([q, months], i) => (
          <button key={q} type="button" data-setq={i + 1} className={st.quarter === i + 1 ? 'on' : ''}
...
function MonthGrid({ st, patch }: { st: DateState; patch: Patch }) {
  return (
    <>
      ...
      <div className="ygrid">
        {MMM.map((m, i) => (
          <button key={m} type="button" data-setmonth={i} className={st.month === i ? 'on' : ''}
    ...
    <>
      ...
      <div className="cgrid">
        ...
        {Array.from({ length: days }, (_, i) => {
          ...
          return (
            <button key={d} type="button" className={cls} data-day={t} onClick={() => pickDay(t)}>{d}</button>
          )
        })}
      </div>
    </>
